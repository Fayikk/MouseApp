...
namespace API.Data
{
    public class UserRepository : IUserRepository
    {
       ...
       public async Task<PagedList<MemberDto>> GetMemberAsync(UserParams userParams)
        ...
        public async Task<BusinessUser> GetUserByIdAsync(int id)
        {
            return await _context.Users.FindAsync(id);
        }

        public async Task<BusinessUser> GetUserByUsernameAsync(string username)
        {
            return await _context.Users
        ...
        public async Task<string> GetUserPosition(string username)
        {
            return await _context.Users
            .Where(x => x.UserName == username)
            .Select(x => x.Position).FirstOrDefaultAsync();
        }

        public async Task<IEnumerable<BusinessUser>> GetUsersAsync()
        {
            return await _context.Users
            .Include(p=>p.Photos)
            .ToListAsync();
